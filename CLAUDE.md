...
- **Продукт:** беспроводной линк (CRSF-управление дроном + видео) между двумя устройствами FPV-наземки (У1 — мастер-пульт с **RadioMaster Boxer ELRS** (заменил TX12) + видео-передатчиком к очкам, У2 — выносная база с 2× ELRS-передатчиками и VRX). Дистанция до 1 км, через готовый Wi-Fi PtP мост TP-Link CPE710.
...
- **`rtpjitterbuffer latency=<env> drop-on-latency=false do-lost=true`** под транспорт. Дефолт `latency=50` (env `JITTER_LATENCY`) в `video_rx.sh` — замер на bench-WG 2026-06-04: порог дропов ~10 мс, 50 = 5× запас. `drop-on-latency=false`, `do-lost=true`. Для CPE710 PtP (3–7 мс air) можно снижать дальше — основной knob glass-to-glass.
...
### 2026-06-26 · PowerShell-коммит невидим Claude Code до перечитки git
Коммиты `c4aced9`/`27502c0` делались в PowerShell-канале; Claude Code в том же дереве несколько раз сообщал «файл ещё M / не закоммичен», хотя коммит уже был в истории — его git-снимок застрял на моменте до коммита.
**Правило:** при смешанном workflow (правки агентом ⟂ коммит в PowerShell) после каждого PowerShell-коммита давать агенту `git log --oneline -2` на пересинхрон, прежде чем доверять его «M / не закоммичено».
**Проверка:** `git cat-file -t <hash>` — если объект есть (`commit`), тревога агента ложная.

### 2026-06-26 · Многострочный git-коммит в PowerShell — два -m, не heredoc
В PowerShell-канале финальный git-блок исполняется в PowerShell, не bash. `printf … > /tmp/msg; git commit -F` там не работает (`printf` не cmdlet, `/tmp/` нет) → `commit -F` падает, `push` тихо говорит «up-to-date».
**Правило:** многострочные commit-сообщения в PowerShell — через два `-m` (`git commit -m "subject" -m "body"`). Кириллица в аргументах PowerShell 7 проходит.
**Проверка:** `git --no-pager log -1 --format='%s%n%b'` — subject и body на месте, кириллица цела.

### 2026-06-13 · UART7 на u2: физ. pin 26 (m2), не 29 — закрыт спор 26-vs-29
`gpio readall` на живой u2: физ. **pin 26** = GPIO1_B5 = **UART7_TX (overlay m2, ALT10)** → `/dev/ttyS7`. SPI0 data/clock (физ. 19/21/23) неактивны → ALT10 на 24/26 = UART7-m2, а не SPI. Путь **m1 (физ. 29/38 = GPIO3_C1/C0)** — обычный `GPIO IN`, НЕ активен (это bidir-вариант с рабочим loopback из `wiring-opi5max.md`, в деплое не задействован).
**Правило:** боевой u2 = UART7-**m2**, TX на физ. pin **26** — рабочая истина; «pin 24/29» из старых снапшотов/памяти — мимо. RX/телеметрия (RSSI на пульт) потребует пути m1 (29/38) или подъёма RX на m2 — отдельная доработка (трек D).
**Проверка:** `sudo gpio readall` на u2 → pin 26 в `ALT10`; `cat /etc/u1u2-bridge/crsf-elrs.env` → `/dev/ttyS7`.

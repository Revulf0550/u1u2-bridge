...
.DESCRIPTION
  ...
  ПЕРЕД запуском (руками):
    1. Обе Pi физически переставлены в место целевого режима:
         ...
         ⚠️ Конец A: между LAN-портом PoE-адаптера CPE-AP и {u1 + ARDOR}
         обязателен мини-свитч (u1/RK3588 GMAC не линкуется напрямую в
         пассивный PoE). Технику — только в порт LAN адаптера, никогда
         в POE / в сам CPE.

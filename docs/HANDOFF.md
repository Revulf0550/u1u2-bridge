...
- Сетевой интерфейс должен называться `end0` (имя по умолчанию на Joshua Riek). Если в новой среде имя другое (`eth0`, `enp1s0`) — править `install.sh` строка 61.
...
|---|---|---|
| Архитектура и BOM | ✅ финализированы | Hardware определён, заказы сделаны |
| `crsf_bridge.py` | ✅ написан, **не тестирован на железе** | Логика проста, должен работать; auto-direction RS485 — открытый вопрос |
| `video_tx.sh` + `video_rx.sh` | ✅ написаны, **не тестированы** | Параметры подбирались по даташитам RK3588 |
| `install.sh` | ✅ написан, **не запускался** | Может вылезти проблема с именем сетевого интерфейса |
...
|---|---|---|
| Установка ОС на Orange Pi | Не начата | Прошить SD/NVMe, базовая настройка, статика, WireGuard |
| Настройка CPE710 | Не начата | Параметры расписаны в STEP-3, осталось применить |
| WireGuard-туннель | Конфиги в инструкции готовы | Запуск, проверка handshake |
| Подключение к RS485 | Адаптеры заказаны | Получить → собрать переходник → проверить /dev/ttyUSB |
...
### 7.3. udev-правила содержат ID для CP2102N, не для CH343G

**Симптом:** В `install.sh` строки 110–117 прописаны `idVendor=10c4, idProduct=ea60` (Silicon Labs CP2102N). После того как пользователь выбрал Waveshare на CH343G — эти ID неверны.

**Что нужно сделать после получения адаптеров:**
```bash
lsusb     # найти строку с WCH USB-RS485, скопировать ID типа "1a86:55d3"
udevadm info -a /dev/ttyUSB0 | grep -m1 'ATTRS{serial}'
```

И пересобрать правила:
```
SUBSYSTEM=="tty", ATTRS{idVendor}=="1a86", ATTRS{idProduct}=="55d3", \
  ATTRS{serial}=="<серийник>", SYMLINK+="ttyUSB-CRSF1"
```

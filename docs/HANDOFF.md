...
> ⚠️ **ВНИМАНИЕ: ДОКУМЕНТ УСТАРЕЛ (на 2026-05-22).** Архитектурные секции и распиновка UART/GPIO здесь не отражают актуальное состояние проекта. По UART/GPIO на Orange Pi 5 Max — единственный источник истины: [`docs/wiring-opi5max.md`](wiring-opi5max.md) (деплой: UART7 overlay `m2`, физ. pin 26; m1/29-38 — резерв, трек D). По актуальным урокам и инцидентам — `CLAUDE.md` в корне репо, раздел "Lessons & Incidents". Переписывание этого HANDOFF — задача отдельной сессии; до тех пор сверяйся с указанными выше документами.

> **NB (2026-06-13, live `gpio readall`):** упоминания «UART7-M2 (pin 26)» в этом доке (≈стр. 954/1096) — **ВЕРНЫ**. Боевой деплой u2 = UART7 overlay m2, TX на физ. pin 26 → `/dev/ttyS7`. НЕ «чинить» под m1/29. Полная картина: CLAUDE.md (урок 2026-06-13) + `wiring-opi5max.md` (баннер 2026-06-13). m1 (29/38) = резерв под RX/телеметрию (трек D).
...
- **u1-pi (Orange Pi 5 Max):** залит joshua-riek 24.04, NVMe-загрузка работает с заводским U-Boot в SPI (на этой плате `u-boot-install-mtd` оказался не нужен — SPI уже был прошит производителем). Hostname `u1-pi`, статика по DHCP на роутере (`192.168.31.72/24`). `install.sh u1` отработал end-to-end с `IFACE=enP3p49s0 SKIP_NETPLAN=1 SKIP_VIDEO=1 PEER_IP_OVERRIDE=10.8.0.7 WAVESHARE_SERIAL_1=5A7C185549 WAVESHARE_SERIAL_2=5A98058254`.
- **Сетевой интерфейс на обеих Pi:** один 2.5GbE, имя `enP3p49s0`. `install.sh` определяет имя автоматически через `ip -br link | awk '$2 == "UP"'`.
- **WireGuard:** обе Pi подключены клиентами к wg-easy на VPS `95.140.147.108`. u2-pi `10.8.0.7/24`, u1-pi `10.8.0.6/24`. Дефолтный конфиг от wg-easy у обоих требовал ручной правки: `AllowedIPs = 10.8.0.0/24` (не `0.0.0.0/0`), `PersistentKeepalive = 15` (не `0`). NSU-pc (`10.8.0.5`) сознательно остался с `AllowedIPs = 0.0.0.0/0` (full-tunnel через VPS — пользовательское решение для bench).
- **Локальная NSU-pc ↔ Pi через WG:** работает в обе стороны. ICMP echo на NSU-pc заблокирован Windows Firewall — для проверки связи с u1-pi/u2-pi использовать SSH (`ssh ubuntu@10.8.0.6`/`10.8.0.7`), не `ping`.
- **ufw на u2-pi:** активен с allowlist для `10.8.0.5` (NSU-pc) и `10.8.0.6` (u1-pi) на 22/tcp + 14550/udp + 14551/udp.
- **ufw на u1-pi:** `Status: inactive`. Для bench-фазы оставлено открытым, в полевой фазе включить симметрично u2-pi.
- **Репозиторий:** `https://github.com/Revulf0550/u1u2-bridge` (public). На обеих Pi склонирован в `~/u1u2-bridge`.
- **RS485-адаптеры Waveshare USB-TO-RS485 (B) на CH343G:** 4 из 4 подключены и зарегистрированы. Драйвер `cdc_acm`, имена `/dev/ttyACMx`, vendor:product `1a86:55d3`. Серийники:

  | Pi | Канал | Серийник | udev symlink |
  |---|---|---|---|
  | u2-pi | CRSF1 | `5A98051690` | `/dev/ttyACM-CRSF1` |
  | u2-pi | CRSF2 | `5A98047217` | `/dev/ttyACM-CRSF2` |
...
### 7.3. Waveshare USB-TO-RS485 (B) на CH343G — `/dev/ttyACMx` (ИСПРАВЛЕНО, обе Pi охвачены)

**Резюме:** Waveshare USB-TO-RS485 (B) при подключении к Orange Pi 5 Max (Ubuntu 24.04) определяется как USB CDC ACM device:

- **vendor:product** = `1a86:55d3` (WCH CH343G)
- **драйвер** = `cdc_acm`
- **имя устройства** = `/dev/ttyACMx` (не `/dev/ttyUSBx` — это для CP210x/CH340G, другие чипы)

**install.sh обновлён под это:** udev SYMLINK `ttyACM-CRSFx`, env `SERIAL_DEV=/dev/ttyACM-CRSFx`, передача серийников через `WAVESHARE_SERIAL_1=... WAVESHARE_SERIAL_2=... sudo ./install.sh ...`, проверка `${SUDO_USER}` в `dialout` с подсказкой, `udevadm trigger --subsystem-match=tty --action=change` после reload (symlinks появляются без реконнекта).

**Известные серийники** — см. таблицу в разделе 6 (Сессия 2026-05-18).

**Команда узнать серийник нового адаптера:**
```bash
udevadm info -a /dev/ttyACM<N> | grep -m1 'ATTRS{serial}'
```

**Урок:** записан в `CLAUDE.md` (2026-05-18 · Waveshare USB-TO-RS485 (B) на CH343G → /dev/ttyACMx).
### 7.4. video_out на плате У-устройств: распиновка жёлтый/белый — РЕШЕНО

**Резюме:** Жёлтый = pin 1 = CVBS сигнал, белый = pin 2 = GND/shield. Подтверждено прозвонкой мультиметром 2026-05-19 при пайке u1-переходника. История: в сессии 2026-05-18 вопрос был временно закрыт в `docs/wiring.md` в **обратной** интерпретации (белый=сигнал, жёлтый=GND) по фото без видимой маркировки. 2026-05-19 пятаки video_out пересняли крупно, на плате обнаружена маркировка «+» (нижний, жёлтый) и «−» (верхний, белый); прозвонка подтвердила. `docs/wiring.md` и таблица распиновки исправлены в одном commit'е с этим обновлением §7.4.

**Финальная распиновка:** см. `docs/wiring.md` (раздел «Распиновка (final)»). Жёлтый → RCA центральная жила. Белый → RCA shell + GND клеммы обоих Waveshare.

**Урок:** записан в `CLAUDE.md` (2026-05-19 · Распиновка платы по фото без прозвонки).
...
**Завершено в сессии 2026-05-18:**

- ✅ Прошить и настроить обе Orange Pi 5 (u1-pi и u2-pi)
- ✅ Поднять WireGuard-туннели обеим Pi (симметрично к wg-easy на VPS NL)
...
5. **Настроить CPE710 в PtP** по параметрам STEP-3 (channel 149, 20 MHz, Bridge mode, distance 1 km). Проверить `ping` и `iperf3` между Pi через мост напрямую (без VPS). ~30 минут.

6. **Полевой переход с VPS на CPE710 LAN:**
   - Включить ufw на u1-pi симметрично u2-pi (allowlist 22/14550/14551 от партнёра)
   - Переключить env-файлы CRSF-мостов с `PEER=10.8.0.x` на локальный `PEER=192.168.1.x` (либо оставить через WG поверх — решим по результатам пингов на дистанции)
   - Заменить IFACE-сетевые настройки на статику внутри 192.168.1.0/24

7. **Подключить четвёртый Waveshare адаптер**, узнать серийник, добавить в env (на ту Pi, где будет CTRL-канал переключения VRX).
